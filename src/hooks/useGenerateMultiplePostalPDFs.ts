...
export const useGenerateMultiplePostalPDFs = () => {
  const generatePostalPDFs = useCallback(
    async (file: File, onProgress?: (percent: number) => void) => {
      try {
        const buffer = await file.arrayBuffer();
        const workbook = XLSX.read(buffer, { type: "array" });
        const sheet = workbook.Sheets[workbook.SheetNames[0]];
        const rows = XLSX.utils.sheet_to_json<postalFormFieldTypes>(sheet);
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          try {
            ...
            /* organization */
            const organization = wrapTextByLength(
              safeText(row.organization),
              11
            );
            ...
            /* bankBranchCode */
            page.drawText(safeText(row.postalCode), {
              x: 180,
              y: 354,
              size: 12,
              font: helveticaFont,
            ...
            const postOfficeAccount = splitStringIntoGroups(
              safeText(row.postOfficeAccount)
            );
